...
export class AccountKey {
   ...
   */
  toString(): string {
    return '{ "account": "' + this.account + '", "allowance": "' + this.allowance.toString() + '", "allowed_functions": "' + this.allowedFunctions.values().toString() + '"}'
  }
}
...
const _PK_DIFFICULTY_PAD = 4 // this can be adjusted to make keys harder to forge. default is 4 -- which means that with the prefix there's 58^10 guesses to forge a random key, and much much harder to forge a specific one.  
const _PK_MAX_ACCOUNT_LENGTH = _PK_LENGTH - _PK_PREFIX.length - _PK_DIFFICULTY_PAD // 

// Not in use
// TODO :: support index'd and nonce'd account keys (multiple keys / pk)
 ...
 * @param account 
 */
export function account_id_to_pk(account: string): Uint8Array {
 ...
 */
export function pk_to_account_id(pk: Uint8Array): string {
  const b58Pk = base58.encode(pk)
  assert(b58Pk.startsWith(_PK_PREFIX), "INVALID PK FORMAT") // disregard difficulty pad ?

  const accountID = b58Pk
...
export function pk_is_account_key(pk: Uint8Array): bool {
  const b58Pk = base58.encode(pk)
  const minPKPrefix = _PK_PREFIX.padEnd(_PK_PREFIX.length + _PK_DIFFICULTY_PAD , "X")
  return b58Pk.startsWith(minPKPrefix)
}